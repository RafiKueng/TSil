...
However, we are also going to fit an ARMA(1,1) and an ARMA(1,2) model, because both of them showed as well very low AICC-values (see AICC-Matrix in figure).\footnote{
    Estimations by the autofit()-function of the "itsmr" package of R (\TODO{\citation{R Core Team}}) suggest as well an MA(1)-model as best model with a very similar estimate for $\theta_1$ and very close AICC.
...
We now estimate coefficients (\cite[p.~83]{bd02}) for these three model candidates   by the arima()-function of the R-package "stats" \TODO{\citation{R Core Team}}.
